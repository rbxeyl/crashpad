...
static void _x86_check_features(void);
static int fake_pthread_once(volatile LONG *once_control_arg,
...
static int fake_pthread_once(volatile LONG *once_control_arg,
    ...
    if (*once_control_arg != 1) {
        while (1) {
            switch (InterlockedCompareExchange(once_control_arg, 2, 0)) {
            case 0:
                init_routine();
                InterlockedExchange(once_control_arg, 1);
                return 0;
            case 1:
